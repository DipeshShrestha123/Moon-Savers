...
function init() {
    ...
    camera = new THREE.PerspectiveCamera(40, (window.innerWidth * 0.6) / window.innerHeight, 0.1, 1000);
    ...
    const texture = new THREE.TextureLoader().load('./texture/moon3.jpg');
...
async function getDotsFromJson() {
    const response = await fetch("https://dipeshshrestha123.github.io/Moon-Savers/data.json");
...
function onMouseMove(event) {
    ...
    if (intersects.length > 0 && intersects[0].object.uuid != lastSelectedObject) {
        ...
        intersects[0].object.material.color = new THREE.Color('purple')
